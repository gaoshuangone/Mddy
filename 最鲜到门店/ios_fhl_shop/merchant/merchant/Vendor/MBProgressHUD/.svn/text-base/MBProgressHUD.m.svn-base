...
+ (MBProgressHUD*)sharedHudView {
    static dispatch_once_t once;
    static MBProgressHUD *sharedHudView;
    dispatch_once(&once, ^ { sharedHudView = [[self alloc] initWithFrame:[[UIScreen mainScreen] bounds]]; });
...
+(void)hudShowWithViewcontroller:(id)viewcontroller
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        MBProgressHUD* hud =(MBProgressHUD*)[viewc.view viewWithTag:1991];
        
        if(hud)
        {
            [self sharedHudView];
            [self hudHideWithViewcontroller:viewcontroller];
            
        }
        
        hud = [[MBProgressHUD alloc] initWithView:viewc.view];
...
#pragma mark 显示活动指示器
+ (void)hudShowWithViewcontrollerStatus:(id)viewcontroller : (NSString *)string
{
    if(!viewcontroller)
    {
        return;
...
//    CGSize labelSize = [label.text sizeWithFont:label.font constrainedToSize:CGSizeMake(200, 300)];
    
    CGRect stringRect = [label.text boundingRectWithSize:CGSizeMake(200, 300) options:NSStringDrawingUsesLineFragmentOrigin attributes:[NSDictionary dictionaryWithObjectsAndKeys:label.font,NSFontAttributeName, nil] context:nil];
    CGSize labelSize = CGSizeMake(stringRect.size.width, stringRect.size.height);
//    labelSize.height = labelSize.height + 250;
