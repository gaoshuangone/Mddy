//
//  OrderColosedViewController.m
//  merchant
//
//  Created by gs on 15/5/11.
...
@interface OrderColosedViewController ()<UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate,AnimateDelegate>
...
@implementation OrderColosedViewController
...
//    }else{
        self.labelPrice.text   = [CommonUtils getDateForStringTime:[self.dictData valueForKey:@"createTime"] withFormat:@"yyyy年MM月dd日 HH:mm:ss"];
//    }
...
//        [cellView addSubview:self.labelSuccessed];
    UILabel* colosed = [[UILabel alloc]init];
    colosed.text = @"关闭时间:";
    colosed.font = UIFont(16);
    [colosed sizeToFit];
   
    if ([[[self.dictData valueForKey:@"collectionTime"] toString] isEqualToString:@"0"]) {
         colosed.center = CGPointMake(10+colosed.bounds.size.width/2, CGRectGetMaxY(labelJinE.frame)+10+colosed.bounds.size.height/2);
        [cellView  addSubview:colosed];
        
        self.labelColosed = [[UILabel alloc]init];
        self.labelColosed.text =    [CommonUtils getDateForStringTime:[[self.dictData valueForKey:@"receivingTime"] toString]withFormat:@"yyyy年MM月dd日 HH:mm:ss"];
        self.labelColosed.font =UIFont(16);
        [self.labelColosed sizeToFit];
        self.labelColosed.center = CGPointMake(CGRectGetMaxX(colosed.frame)+15+self.labelColosed.bounds.size.width/2, CGRectGetMidY(colosed.frame));
        [cellView  addSubview:self.labelColosed];
    }else{
        
    UILabel* labelFaDan = [[UILabel alloc]init];
    labelFaDan.text = @"揽收时间:";
    ...
    self.labelSend.text =     [CommonUtils getDateForStringTime:[[self.dictData valueForKey:@"collectionTime"] toString]withFormat:@"yyyy年MM月dd日 HH:mm:ss"];
    ...
    [cellView  addSubview:self.labelSend];
        ...
        colosed.center = CGPointMake(10+colosed.bounds.size.width/2, CGRectGetMaxY(labelFaDan.frame)+10+colosed.bounds.size.height/2);
        [cellView  addSubview:colosed];
        self.labelColosed = [[UILabel alloc]init];
        self.labelColosed.text =    [CommonUtils getDateForStringTime:[[self.dictData valueForKey:@"receivingTime"] toString]withFormat:@"yyyy年MM月dd日 HH:mm:ss"];
        self.labelColosed.font =UIFont(16);
    ...
    if ( [[self.dictData valueForKey:@"killedReason"] toString].length!=0) {
    
    UILabel* killed = [[UILabel alloc]init];
    killed.text = @"关闭原因:";
    killed.font = UIFont(16);
    [killed sizeToFit];
    killed.center = CGPointMake(10+killed.bounds.size.width/2, CGRectGetMaxY(colosed.frame)+10+killed.bounds.size.height/2);
    [cellView  addSubview:killed];
   
    self.killedReason = [[UILabel alloc]init];
//           self.killedReason.text = @"热饮，小心烫口，先喝第一口汤，在啃鸭脖，然后再喝第二口汤。";
    self.killedReason.text = [[self.dictData valueForKey:@"killedReason"] toString];
    self.killedReason.font = UIFont(16);
    self.killedReason.numberOfLines = 2;
    self.killedReason.lineBreakMode = NSLineBreakByWordWrapping;
//    CGSize size2 = CGSizeMake(WINDOW_WIDTH-100,170);
//    CGSize labelsize2 = [self.killedReason.text  sizeWithFont:self.killedReason.font constrainedToSize:size2 lineBreakMode:NSLineBreakByWordWrapping];
        
    NSAttributedString *attributedText =
    [[NSAttributedString alloc]
         initWithString:self.killedReason.text
         attributes:@
         {
    ...
    self.killedReason.frame = CGRectMake(CGRectGetMaxX(killed.frame)+15,(iPhone6Plus?-2:0)+ CGRectGetMidY(killed.frame)-2-6, labelsize2.width, labelsize2.height);
    [cellView addSubview:self.killedReason];
...
//    [cellView  addSubview:self.labelSuccessed];
    ...
    if ([[[self.dictData valueForKey:@"quickType"] toString] isEqualToString:@"2"]  && [[self.dictData valueForKey:@"content"] toString].length!=0) {
       ...
       if ( [[self.dictData valueForKey:@"killedReason"] toString].length!=0) {
        labelAddress.center = CGPointMake(10+labelAddress.bounds.size.width/2, CGRectGetMaxY(self.killedReason.frame)+8+labelAddress.bounds.size.height/2);
       }else{
            labelAddress.center = CGPointMake(10+labelAddress.bounds.size.width/2, CGRectGetMaxY(colosed.frame)+8+labelAddress.bounds.size.height/2);
       }
...
//        CGSize size3 = CGSizeMake(WINDOW_WIDTH-100,170);
//        CGSize labelsize3 = [self.labelAddressed.text  sizeWithFont:self.labelAddressed.font constrainedToSize:size3 lineBreakMode:NSLineBreakByWordWrapping];
        ...
        CGRect rect = [attributedText boundingRectWithSize:(CGSize){WINDOW_WIDTH-100, 170}
                                                   options:NSStringDrawingUsesLineFragmentOrigin
                                                   context:nil];
        CGSize labelsize3 = rect.size;
        self.labelAddressed.frame = CGRectMake(CGRectGetMaxX(labelAddress.frame)+15,(iPhone6Plus?-2:0)+ CGRectGetMidY(labelAddress.frame)-2-6, labelsize3.width, labelsize3.height);
        ...
        NSLog(@"%ld",(unsigned long)(unsigned long)[[self.dictData valueForKey:@"killedReason"] toString].length);
            if ( [[self.dictData valueForKey:@"killedReason"] toString].length!=0) {
                
                 viewLine2   = [[UIView alloc]initWithFrame:CGRectMake(10, CGRectGetMaxY(self.killedReason.frame)+8, WINDOW_WIDTH-20, 01)];
            }else{
           viewLine2   = [[UIView alloc]initWithFrame:CGRectMake(10, CGRectGetMaxY(colosed.frame)+8, WINDOW_WIDTH-20, 01)];
            }
...
//        }else{
            ...
            UIImageView* imageView22 = [[UIImageView alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.labelPhoneNumber.frame), CGRectGetMaxY(self.labelAddr.frame)+3, 24, 24)];
            imageView22.image = [UIImage imageNamed:@"orderform_call_.png"];
            [cellView addSubview:imageView22];
...
//    UIImageView* cellImageView1 = [UIImageView circleImageViewFrame:CGRectMake(10, 40, 40,40) Radius:40/2];
//    __weak UIImageView* cellImageView =   cellImageView1;
//    //        cellImageView.image = [ UIImage imageNamed:@"logo.png"];
//    [SelfUser mddyRequestWIthImageWithBrandID:[self.dictData valueForKey:@"brandIdUrl"] withBlock:^(UIImage * image, NSError *error) {
//        if (!error) {
//            if (image == nil) {
//                cellImageView.image = [UIImage imageNamed:@"business_default_photo_small"];
...
//    }
    ...
    //
    //        self.buttonTEL = [UIButton buttonWithType:UIButtonTypeCustom];
    //        [self.buttonTEL setTitle:[NSString stringWithFormat:@"配  送  员:   %@",[[self.dictData valueForKey:@"courierName"] toString]] forState:UIControlStateNormal];
    //        _buttonTEL.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
    //        [self.buttonTEL setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    //        self.buttonTEL.titleLabel.font = UIFont(16);
    //        //        [self.buttonTEL addTarget:self action:@selector(tel) forControlEvents:UIControlEventTouchUpInside];
